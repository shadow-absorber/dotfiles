...
NAME="screenshot_$(date +%F_%H.%M.%S).jpg"
...
export GRIMBLAST_EDITOR="$(echo 'swappy -f')"
...
option_capture_4="Capture Window"
...
# Rofi CMD
wofi_cmd() {
    wofi --dmenu -i -p "Take screenshot"
}
...
# CMD
type_screenshot_cmd() {
    wofi --dmenu -i -p "Type of screenshot"
}

# Ask for confirmation
type_screenshot_exit() {
  echo -e "$option_capture_1\n$option_capture_2\n$option_capture_3\n$option_capture_4" | type_screenshot_cmd
}
